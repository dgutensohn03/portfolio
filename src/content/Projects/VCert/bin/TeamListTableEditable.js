...
auth.onAuthStateChanged(function (user) {
  ...
  $('#datePopup').unbind().click(function () {
    ...
    var end = new Date(date.getFullYear(), date.getMonth(), date.getDate()); //console.log(today)

    var HDDates = db.ref('/HDClass').orderByChild("Value"); //   HDDates.on('child_removed', function(data) {
    ...
    HDDates.once('value', function (snapshot) {
      snapshot.forEach(function (data) {
        ...
        function formatDate(inputDate) {
          var dateString = inputDate.split("-"); //console.log(inputDate)

          var year = dateString[0];
          var month = dateString[1];
          var day = dateString[2]; //console.log('year: ' + year)
      ...
      $('.add-new-btn-hd').unbind().click(function () {
        ...
        $(".HDClass" + newItemIndex + ' .input-group input').on('changeDate', function () {
          $(this).datepicker('update');
          var itemIndex = $(this).parent().parent().attr("class").split(' ').pop().slice(7);
          var itemValue = $(this).val(); //dateChanged = $(this).val();
    ...
    var LDDates = db.ref('/LDClass').orderByChild("Value"); // LDDates.on("child_added", function (data) {
    ...
    var OHWDates = db.ref('/OHWClass').orderByChild("Value"); // VSPDates.on("child_added", function (data) {
    // 	var item = $('<div class="panel-body"></div>').val(data.val()).html(data.key);
    ...
    var VPSDates = db.ref('/VPSClass').orderByChild("Value"); // VSPDates.on("child_added", function (data) {
    // 	var item = $('<div class="panel-body"></div>').val(data.val()).html(data.key);
    // 	$("#VSPDates").append($('<div class="panel-body"></div>').val(data.val()).html(data.key));
    // 	$(item).append('<div class="edit-date">EDIT</div>');
    // 	$("#VSPDates").append(item);
    ...
    function attachDateControls() {
      ...
      $('.edit-date').unbind().click(function () {
        ...
        $(this).parent().find('.cancel-date-box').show(); //$(this).parent().find('.date-item').attr('contenteditable', 'true');
      ...
      $('.save-date-btn').unbind().click(function () {
        ...
        if (oldDate != newDate) {
          var reformatDate = function reformatDate(inputDate) {
            var dateString = inputDate.split("-");
            console.log(inputDate);
            var year = dateString[2];
            var month = dateString[0];
            var day = dateString[1]; //console.log('year: ' + year)
            //console.log('month: ' + month)
            //console.log('day: ' + day)
          ...
          var updatedDate = reformatDate(newDate); //console.log(updatedDate)

          console.log('NEW ENTRY: ' + newDate + ": " + updatedDate);
          db.ref('/' + classRef + 'Class').update(_defineProperty2({}, newDate, updatedDate)).then(function (success) {}); // end update database
          ...
          ref2.once("value").then(function (snapshot) {
            if (oldDate) {
              ...
              if (childKey) {
                var oldRef = firebase.database().ref('/' + classRef + 'Class').child(oldDate);
                oldRef.remove();
              }
            }
