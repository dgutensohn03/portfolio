...
var dayArrayLD = [
    {days:0,title:"10,000 Mile Drain Extension", implemented:false},
    {days:14,title:"Value to Customer", implemented:false},
    {days:28,title:"Engine Oil Advantage", implemented:false},
    {days:42,title:"One Solution™", implemented:false},
    {days:56,title:"ZEREX™ Approvals", implemented:false},
    {days:70,title:"ZEREX™ Nitrite Free Coolant", implemented:false},
    {days:84,title:"Transmission Fluids", implemented:false},
    {days:98,title:"Gear Oils", implemented:false},
    {days:112,title:"HD Grease", implemented:false},
    {days:126,title:"Why Valvoline™", implemented:false}
];
var dayArrayVPS = [
    {days:0,title:"10,000 Mile Drain Extension", implemented:false},
    {days:14,title:"Value to Customer", implemented:false},
...
function setChallengeProgressIndicators(snapshot, thisClass)
{
  $.each(snapshot.child('c' + thisClass).val(), function (index, value) 
  {
    ...
    var dayZero = convertToDate(snapshot.child("ClassCompletion" + thisClass).val());
    console.log("Current Class Start Date: " + dayZero);
    ...
    if (!index.includes("points") && !index.includes("first") && !index.includes("second") && !index.includes("third")) 
    {
      ...
      var unlockDate = new Date(dayZero.getTime());
      var unlockTime = new Date(dayZero.getTime());
      // console.log("challenge number: " + cNum);
...
function populateLeaderboard(snapshot, thisClass)
{
  ...
  var onLBHD = allUsers.once("value", function (data4) {
    ...
    if(truncatedLB&&!youOnLB)
    {
      ...
      var totalPoints = snapshot.child('TotalPoints' + thisClass).val();
      // console.log(firstName)
      // console.log(lastName)
      // console.log(totalPoints)
      $("#" + thisClass + "Leader .expandedSection", $("#contents").contents()).append(`<div class="userRow">
        <div class="sellerPlace">YOU</div>
        <div class="sellerName">${firstName + " " + lastName}</div>
        <div class="scoreText">
            <font class="highScore">${totalPoints}</font>
            <font class="pointsText">POINTS</font>
        </div>
        </div>`);
    }
