  ...
  if (snapshot.child("Class" + thisClass).val()) {
    ...
    var onLB = allUsers.once("value", function (data4) {
      ...
      rankings.forEach(function(child){
        ...
        console.log('here',snapshot.child("Email").val())
        $("#" + thisClass + "Leader .expandedSection").append(`<div class="${place}">
...
function onOverCanvas(e)
{
  ...
  let p = {x:130, y:130};
  let x = p.x - (e.pageX - $(e.currentTarget).offset().left);
  let y = p.y - (e.pageY - $(e.currentTarget).offset().top);
...
{
  let leaderPercent = 100 - Math.floor(((leaderRadians + 0.5) / 2) * 100);
  let scoredPercent = 100 - Math.floor(((scoredRadians + 0.5) / 2) * 100);
  ctx.beginPath();
  ctx.fillStyle = "white";
...
function getCorrectTail(thisPlace) {
  const num = parseInt(thisPlace, 10);
