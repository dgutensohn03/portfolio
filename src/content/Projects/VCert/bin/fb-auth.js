...
auth.onAuthStateChanged(function (user) {
  if (user) {
    ...
    var userId = firebase.auth().currentUser.uid; //var isDemoMode = checkIfDemoMode(userId);
...
function checkIfDemoMode(userId) {
  return firebase.database().ref(dbLocation).child(userId).once('value').then(function (snapshot) {
    if (snapshot.exists()) {
      var userData = snapshot.val();
      console.log("User data:", userData); // Assume 'isDemoMode' is a property in the user data
...
$('#login-form').on('submit', function (e) {
  ...
  auth.signInWithEmailAndPassword(email, password).then(function (cred) {//document.getElementById('login-form').reset();
    ...
    if (password.toLowerCase() == "valvoline") {
      ...
      parent.navigateIframe('portal.html'); //$("#contents", window.parent.document).attr("src", "portal.html");
    }
  })["catch"](function (error) {
...
$('#signup-form').submit(function (e) {
  ...
  var regionProvince = document.getElementById('regionProvince').value; //var consentCheckbox = document.getElementById('consentCheckbox').checked;
  ...
  var monthlyGoalXP = 0;
  var goalXP = 0;
  var totalPoints = 0; //check if required fields have values if not element is tagged with is-invalid class
    ...
    if (consentCheckbox) {
      if (password === passwordConfirm && password != "" && passwordConfirm != "") {
        secondaryApp.auth().createUserWithEmailAndPassword(email, password).then(function (cred) {
          ...
          console.log(userId); //create entry in database
          ...
          firebase.database().ref(dbLocation).child(userId).set({
            ...
            $("#registrationPopup").modal('show'); // $("#contents", window.parent.document).attr("src", "portal.html");
  ...
  auth.onAuthStateChanged(function (user) {
    if (user) {
      ...
      ref.once("value").then(function (snapshot) {
        var childKey = snapshot.child("FirstName").val(); //console.log('childKey: ' + childKey);
      });
      $('.auth-function').hide();
      $('#log-out').removeClass('hidden').css('display', 'block');
      $('#home').css('cursor', 'pointer').attr('onclick', 'navigateIframe("LandingPage.html")'); // $('#challenge-btn').css('cursor', 'pointer').attr('onclick', 'navigateIframe("portal.html")');
      ...
      return firebase.database().ref('/' + dbLocation + '/' + userId).once('value').then(function (snapshot) {
        ...
        var hasAdminRights = snapshot.val() && snapshot.val().admin || false; //console.log(hasAdminRights)
...
function markPPAsViewed() {
  ...
  $('#privacyBG > .closeButton', $("#contents").contents()).css('display', 'block'); //   window.open(
...
function addtoMonthlyXP(pointValue) {
  firebase.auth().onAuthStateChanged(function (user) {
    if (user) {
      var getVal = function getVal(userId) {
        return firebase.database().ref('/' + dbLocation + '/' + userId).once("value").then(function (snapshot) {
    ...
    if (user) {
      var getVal = function getVal(userId) {
        return firebase.database().ref('/' + dbLocation + '/' + userId).once("value").then(function (snapshot) {
          var FBdata = [];
          var FBref = [];
          var dbHasThisChallenge = snapshot.child("c" + whichClass).hasChild("c" + challengeNumber + whichClass); //Get value from Firebase Database and push it to array for reference

          FBref.push(snapshot.child("c" + whichClass).val());
          FBdata.push(dbHasThisChallenge); ////console.log(FBdata[0]);
      ...
      ;
      getVal(userId).then(function (FBdata) {
        var hasTaken = FBdata.pop(); //console.log('hasTaken: ' + hasTaken)
        ...
        firebase.database().ref(dbLocation).child(userId).child("c" + whichClass).once("value", function (snapshot2) {
          if (snapshot2.val()["c" + challengeNumber + whichClass] != "1" && snapshot2.val()["c" + challengeNumber + whichClass + "second"] != "1") {
            var updateObj = _defineProperty({}, "c" + challengeNumber + whichClass, bookmark);
    ...
    if (user) {
      var getVal = function getVal(userId) {
        return firebase.database().ref('/' + dbLocation + '/' + userId).once("value").then(function (snapshot) {
          ...
          console.log(); //Get value from Firebase Database and push it to array for reference
          ...
          for (var i = 0; i < FBref.length; i++) {
            //console.log(FBref[i])
            for (var key in FBref[i]) {
              if (key.toString().indexOf("points") != -1) {
                if (key.toString().indexOf(challengeNumber.toString()) != -1) {} else {
          ...
          FBdata.push(totalPointsForClass); ////console.log(FBdata[0]);
      ...
      getVal(userId).then(function (FBdata) {
        ...
        if (hasTaken) {} //console.log('hasTaken: ' + hasTaken)
        //console.log('updatePointValue ' + updatePointValue)
        //console.log('totalPointsForClass ' + totalPointsForClass)
        //console.log('totalUpdatedPoints ' + totalUpdatedPoints)
        //If the challenge in question is finished and replayed 'for fun', don't overwrite progress.
        ...
        firebase.database().ref(dbLocation).child(userId).once("value", function (snapshot2) {
          if (snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass] != "1" && snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "second"] != "1" && snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "third"] != "1" && snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "third"] != "0" && passed) {
            var _updateObj2;

            var updateObj = (_updateObj2 = {}, _defineProperty(_updateObj2, "c" + challengeNumber + whichClass, bookmark), _defineProperty(_updateObj2, "c" + challengeNumber + whichClass + "points", pointValue), _updateObj2); //Check attempt and send points 
            ...
            firebase.database().ref(dbLocation).child(userId).child("c" + whichClass).update(updateObj).then(function () {
              ...
              if (snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "first"] !== undefined) {
                ...
                if (snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "second"] !== undefined) {
                  ...
                  if (snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "third"] !== undefined) {
                    //Challenge closed
                    var updateObj = _defineProperty({}, "c" + challengeNumber + whichClass + "third", 0);
                  } else {
                    var updateObj = _defineProperty({}, "c" + challengeNumber + whichClass + "third", 1);
                  }
                } else {
                  var updateObj = _defineProperty({}, "c" + challengeNumber + whichClass + "second", 1);
                }
              } else {
                var updateObj = _defineProperty({}, "c" + challengeNumber + whichClass + "first", 1);
              }

              firebase.database().ref(dbLocation).child(userId).child("c" + whichClass).update(updateObj); //Check for 3-Peat bonus conditions - At least challenge 3, c(i-1)HD = 1, c(i-2)HD = 1, three(i)HD undefined, three(i-1)HD undefined, three(i-2)HD undefined, passed current challenge

              if (challengeNumber >= 3 && snapshot2.child("c" + whichClass).val()["c" + (challengeNumber - 1) + whichClass] == 1 && snapshot2.child("c" + whichClass).val()["c" + (challengeNumber - 2) + whichClass] == 1 && snapshot2.val().Notifications["three" + challengeNumber + whichClass] == undefined && snapshot2.val().Notifications["three" + (challengeNumber - 1) + whichClass] == undefined && snapshot2.val().Notifications["three" + (challengeNumber - 2) + whichClass] == undefined && passed) {
                var updateObj = _defineProperty({}, "three" + challengeNumber + whichClass, 1);

                firebase.database().ref(dbLocation).child(userId).child("Notifications").update(updateObj);
                updateObj = _defineProperty({}, "three" + challengeNumber + whichClass + "points", 20);
              ...
              firebase.database().ref(dbLocation).child(userId).update(_defineProperty({}, "TotalPoints" + whichClass, totalUpdatedPoints)).then(function () {
                parent.navigateIframe("portal.html");
                console.log('here now');
              });
            }); //if failed
          } else if (snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass] != "1" && snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "second"] != "1" && snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "third"] != "1" && snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "third"] != "0" && !passed) {
            var _updateObj9;

            var updateObj = (_updateObj9 = {}, _defineProperty(_updateObj9, "c" + challengeNumber + whichClass, bookmark), _defineProperty(_updateObj9, "c" + challengeNumber + whichClass + "points", 0), _updateObj9);
            firebase.database().ref(dbLocation).child(userId).child("c" + whichClass).update(updateObj).then(function () {
              //Check if first attempt is in database
              if (snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "first"] !== undefined) {
                //Check if second attempt is in database
                if (snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "second"] !== undefined) {
                  //Check if third attempt is in database
                  if (snapshot2.child("c" + whichClass).val()["c" + challengeNumber + whichClass + "third"] !== undefined) {//Challenge closed
                  } else {
                    sendFailEmail();
                  ...
                  var updateObj = _defineProperty({}, "c" + challengeNumber + whichClass + "second", retakeDate.toString());
                }
                ...
                var updateObj = _defineProperty({}, "c" + challengeNumber + whichClass + "first", retakeDate.toString());
              ...
              function sendFailEmail() {
                console.log('sendFailed email to: ' + snapshot2.val().Email); // $.get("https://us-central1-vcert-9568e.cloudfunctions.net/sendFailed", {
                //   theEmail: snapshot2.val().Email
                // });
              }
            });
...
function checkForPoints(scale) {
  console.log('scaling points based on attempt'); // if (document.getElementsByTagName("iframe")[0].contentWindow.questionPointsPossible) {
  ...
  if (document.getElementsByTagName("iframe")[0].contentWindow.points) {
    var divideByHalf = function divideByHalf(item, index) {
...
function navigateUserLoggedInCheck() {
  auth.onAuthStateChanged(function (user) {
    if (user) {
      ...
      parent.navigateIframe('LoginPage.html'); //$("#contents", window.parent.document).attr("src", "./LoginPage.html");
