...
function setChallengeProgressIndicators(snapshot, thisClass) {
  $.each(snapshot.child('c' + thisClass).val(), function (index, value) {
    ...
    console.log("Current Class Start Date: " + dayZero); //If not taken
    ...
    if (!index.includes("points") && !index.includes("first") && !index.includes("second") && !index.includes("third")) {
      ...
      unlockDate.setDate(dayZero.getDate() + window["dayArray" + thisClass][cNum - 1].days);
      console.log("unlockDate: " + unlockDate); //console.log('Day of challenge unlock: ' + window["dayArray" + thisClass][cNum - 1].days);
      ...
      console.log('timeUntilRetry: ' + timeUntilRetry); //console.log(Math.floor((todaysDate - unlockDate) / 86400000))
      ...
      if (snapshot.child('c' + thisClass).val()[index + "first"] != undefined) {
        if (snapshot.child('c' + thisClass).val()[index + "second"] != undefined) {
          if (snapshot.child('c' + thisClass).val()[index + "third"] != undefined) {
            ...
            retryAvailableUntil = Date.parse(snapshot.child('c' + thisClass).val()[index + "second"]) + retryAvailableFor * 86400000;
            ...
            timeUntilRetry = Date.parse(snapshot.child('c' + thisClass).val()[index + "second"]) - todaysDate - coolDown * 86400000;
            console.log('retryAvailable: ' + retryAvailable);
          ...
          retryAvailableUntil = Date.parse(snapshot.child('c' + thisClass).val()[index + "first"]) + retryAvailableFor * 86400000;
          console.log(Date.parse(todaysDate));
          console.log(Date.parse(snapshot.child('c' + thisClass).val()[index + "first"]));
          console.log('retryAvailableUntil: ' + retryAvailableUntil);
          ...
          timeUntilRetry = Date.parse(snapshot.child('c' + thisClass).val()[index + "first"]) - todaysDate;
          console.log('retryAvailable: ' + retryAvailable);
          console.log('timeUntilRetry: ' + timeUntilRetry);
        }
      } else {
      ...
      console.log('daysLeft: ' + daysLeft); //var timeLeft = Math.floor((todaysDate - unlockDate) / 86400000);
      ...
      if (timeLimitInitialAttempt - 1 - Math.floor((todaysDate - unlockDate) / 86400000) >= timeLimitInitialAttempt && window["dayArray" + thisClass][cNum - 1].implemented) {
        console.log('Course to be unlocked. Only applies to first three challenges at the moment'); //Course to be unlocked. Only applies to first three challenges at the moment.
        ...
        console.log('Course is closed due to timeout.'); //Course is closed due to timeout.
        ...
        console.log('Course available, not started'); //Course available, not started

        $("#" + index, $("#contents").contents()).css('background', 'linear-gradient(90deg, #FFFFFF 100%, #78DD36 0)');
      } else if (value == 1) {
        console.log('Course finished'); //Course finished
        ...
        console.log('Course started, not finished'); //Course started, not finished
        ...
        console.log('Challenge taken once, started but not finished during second attempt'); //Challenge taken once, started but not finished during second attempt
        ...
        if (todaysDate >= Date.parse(snapshot.child('c' + thisClass).val()[index + "first"]) && retryAvailable && snapshot.child('c' + thisClass).val()[index + "second"] === undefined || todaysDate >= Date.parse(snapshot.child('c' + thisClass).val()[index + "second"]) && retryAvailable && snapshot.child('c' + thisClass).val()[index + "third"] === undefined && snapshot.child('c' + thisClass).val()[index + "third"] != 0) {
          console.log("case 1");
          console.log(Date.parse(snapshot.child('c' + thisClass).val()[index + "first"])); //Challenge taken once, available to take second time, user has not attempted yet.
          ...
          console.log(snapshot.child('c' + thisClass).val()[index + "third"] === undefined); //Challenge taken once and failed, in weeklong cooldown period until second attempt.

          daysLeft = Math.ceil(timeUntilRetry / 86400000);
          ...
          console.log("case 3"); //Challenge taken once and failed, in weeklong cooldown period until second attempt.

          daysLeft = Math.floor((Date.parse(snapshot.child('c' + thisClass).val()[index + "second"]) - todaysDate) / 86400000 + timeUntilRetry);
          console.log('daysLeft: ' + daysLeft);
          ...
          console.log("case 4"); //Challenge taken twice, now locked for good.
