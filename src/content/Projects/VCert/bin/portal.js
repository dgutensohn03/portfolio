...
var dayArrayOHW = [{
  ...
  title: "An Oil Made for Off-Highway",
  implemented: true,
  skip: false
}, {
  days: challengeStartDate[1],
  title: "The Grease for Machine (and Business) Success",
  implemented: true,
  skip: false
}, {
  days: challengeStartDate[2],
  title: "A Coolant For Every Situation",
  implemented: true,
  skip: false
}, {
  days: challengeStartDate[3],
  title: "An Innovative Approach to Off-Highway Lubricants",
  implemented: true,
  skip: false
}, {
  days: challengeStartDate[4],
  title: "A Construction Site Tour",
  implemented: true,
  skip: false
}, {
  days: challengeStartDate[5],
  title: "Off-Highway Trends",
  implemented: true,
  skip: false
}, {
...
var dayArrayAD = [{
  days: challengeStartDate[0],
  title: "What Dealers Are Looking For",
  implemented: true,
  skip: false
}, {
  days: challengeStartDate[1],
...
var dayArrayVPS = [{
  ...
  days: challengeStartDate[6],
  title: "15k Restoration Kit/Pop & Pours/Low Touch",
  implemented: true,
  skip: false
}, {
  days: challengeStartDate[7],
  ...
  firebase.auth().onAuthStateChanged(function (user) {
    if (user) {
      ...
      ref.once("value").then(function (snapshot) {
        ...
        console.log("ClassOHW", snapshot.child("ClassOHW").val() === true);

        if (snapshot.child("ClassOHW").val() === true) {
          checkIfInClass(snapshot, "OHW");
          setChallengeProgressIndicators(snapshot, "OHW");
          populateLeaderboard(snapshot, "OHW");
        } else {
          $("#OHWWidget", $("#contents").contents()).css('display', 'none');
          $("#OHWLeader", $("#contents").contents()).css('display', 'none');
        }

        console.log("ClassHD", snapshot.child("ClassHD").val() === true);

        if (snapshot.child("ClassHD").val() === true) {
          checkIfInClass(snapshot, "HD");
          setChallengeProgressIndicators(snapshot, "HD");
          populateLeaderboard(snapshot, "HD");
        } else {
          $("#HDWidget", $("#contents").contents()).css('display', 'none');
          $("#HDLeader", $("#contents").contents()).css('display', 'none');
        }

        console.log("ClassLD", snapshot.child("ClassLD").val() === true);

        if (snapshot.child("ClassLD").val() === true) {
          checkIfInClass(snapshot, "LD");
          setChallengeProgressIndicators(snapshot, "LD");
          populateLeaderboard(snapshot, "LD");
        } else {
          $("#LDWidget", $("#contents").contents()).css('display', 'none');
          $("#LDLeader", $("#contents").contents()).css('display', 'none');
        }

        console.log("ClassVPS", snapshot.child("ClassVPS").val() === true);

        if (snapshot.child("ClassVPS").val() === true) {
          checkIfInClass(snapshot, "VPS");
          setChallengeProgressIndicators(snapshot, "VPS");
          populateLeaderboard(snapshot, "VPS");
        } else {
          $("#VPSWidget", $("#contents").contents()).css('display', 'none');
          $("#VPSLeader", $("#contents").contents()).css('display', 'none');
        }

        console.log("ClassAD", snapshot.child("ClassAD").val() === true);

        if (snapshot.child("ClassAD").val() === true) {
          checkIfInClass(snapshot, "AD");
          setChallengeProgressIndicators(snapshot, "AD");
          populateLeaderboard(snapshot, "AD");
        } else {
          $("#ADWidget", $("#contents").contents()).css('display', 'none');
          $("#ADLeader", $("#contents").contents()).css('display', 'none');
        }
      });
    }
  });
  ...
  firebase.auth().onAuthStateChanged(function (user) {
    if (user) {
      ...
      ref.once("value").then(function (snapshot) {
        ...
        console.log('Current user ClassCompletion' + thisClass + ' date value: ' + snapshot.child("ClassCompletion" + thisClass).val()); //unlockDate.setDate(dayZero.getDate() + window["dayArray" + thisClass][cNum - 1].days);
...
function checkIfInClass(snapshot, thisClass) {
  ...
  console.log('snapshot.child("Class" + thisClass).val()', snapshot.child("Class" + thisClass).val());
  ...
  console.log(dayZero <= todaysDate);
  ...
  if (snapshot.child("Class" + thisClass).val()) {
    ...
    if (snapshot.child("Class" + thisClass).val() && dayZero <= todaysDate) {
...
function setChallengeProgressIndicators(snapshot, thisClass) {
  ...
  $.each(snapshot.child('c' + thisClass).val(), function (index, value) {
    ...
    console.log('indexId.includes(thisClass)', indexId.includes(thisClass));

    if (indexId.includes(thisClass) && !indexId.includes("points") && !indexId.includes("first") && !indexId.includes("second") && !indexId.includes("third")) {
      var cNum = Number(indexId.replace(/[^0-9]+/g, ''));
      console.log('Processing challenge ' + cNum + ' for track ' + thisClass);
      console.log(window["dayArray" + thisClass][cNum - 1]);

      if (window["dayArray" + thisClass][cNum - 1].skip) {
        console.log("Skipping challenge " + cNum);
        $("#" + thisClass + "Widget .sectionTask:eq(" + (cNum - 1) + "), " + "#" + thisClass + "Widget .sectionHorizontalLine:eq(" + (cNum - 1) + ")", $("#contents").contents()).hide();
      }
...
function updateChallengeUI(thisClass, indexId, cNum, challengeState, unlockDate, value) {
  ...
  console.log('Updating UI for challenge ' + thisClass + ' ' + cNum + ' with state: ' + challengeState);
...
function getChallengeState(snapshot, thisClass, indexId, unlockDate, value) {
  ...
  var classTrackLength = window["dayArray" + thisClass].filter(function (item) {
    return item.implemented === true;
  }).length;
  console.log('classTrackLength', classTrackLength);

  function hasWeeksPassed(startDate, numberOfWeeks) {
    // Convert weeks to days
    var numberOfDays = numberOfWeeks * 7; // Get the current date

    var currentDate = new Date(); // Create a new date by adding the number of days to the start date

    var targetDate = new Date(startDate);
    targetDate.setDate(targetDate.getDate() + numberOfDays);
    console.log('currentDate', currentDate);
    console.log('targetDate', targetDate); // Compare the current date with the target date

    if (currentDate >= targetDate) {
      console.log('challenge state - opened');
      return true;
    } else {
      console.log('challenge state - closed');
      return false;
    }
  } // Example usage


  var startDate = snapshot.child("ClassCompletion" + thisClass).val();
  ; // Start date in YYYY-MM-DD format

  var numberOfWeeks = classTrackLength; // Number of weeks

  console.log(hasWeeksPassed(startDate, numberOfWeeks)); // Output will be true or false

  if (hasWeeksPassed(startDate, numberOfWeeks) == true) {
    return "closed";
  }
...
function populateLeaderboard(snapshot, thisClass) {
  ...
  var onLBHD = allUsers.once("value", function (data4) {
    ...
    data4.forEach(function (value) {
      var thisVal = value.val(); //console.log(thisVal); //console.log(data4.value)
      //console.log("data4")
